...
graph TD
    ...
    C -->|LLM request| F[Anthropic API - Claude Sonnet 4.6]
...
flowchart TD
    ...
    SL4 --> SL5[Restart Gateway - Slack connected]
...
```mermaid
flowchart TD
    E1[auth store lock busy] -->|pkill openclaw + delete .auth.lock| F1[Retried onboard]
    E2[openclaw.json not found] -->|Created config manually| F2[Gateway started]
    E3[gateway.mode missing] -->|Added gateway.mode local to config| F3[Validation passed]
    E4[Invalid config - Invalid input] -->|doctor --fix revealed old Node| F4[Upgraded to Node 24]
    E5[node sqlite missing] -->|nvm install 24 + nvm use 24| F5[SQLite available]
    E6[EADDRINUSE port 18789] -->|openclaw gateway stop| F6[Single instance running]
    E7[Wrong skills - wrong language] -->|clawhub install local-file-rag-basic| F7[Correct RAG skill]
    E8[Low Anthropic credits] -->|Added credits at console.anthropic.com| F8[API requests working]
...
sequenceDiagram
    ...
    Slack->>Gateway: WebSocket message via Socket Mode
    Gateway->>RAG: local_file_rag_search with query and rootDir
    RAG->>Docs: Semantic search across PDFs
    Docs-->>RAG: Relevant chunks and source filenames
    RAG-->>Gateway: Structured results with metadata
    Gateway->>Claude: Prompt plus retrieved context
...
| `~/Library/LaunchAgents/com.openclaw.gateway.plist` | macOS daemon |
